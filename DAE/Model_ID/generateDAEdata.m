...
rng(0)
shuffleIdx = randi([1, nComb], 1, nComb);
uSequence = uSequence(:,shuffleIdx);
uSequence = [[3.;1.], uSequence];
uSequence = imresize(uSequence, [2, nComb*DT], 'nearest');

% Exclude low power & high flow --> low temp
idx1 = find(uSequence(1,:)<=min(P_set_vals)+2);
idx2 = find(uSequence(2,:)>=max(q_vals)-2);
idx = intersect(idx1, idx2);
uSequence(:, idx) = [];

% Exclude high power & low flow --> high temp
idx1 = find(uSequence(1,:)>=max(P_set_vals)-1);
idx2 = find(uSequence(2,:)<=min(q_vals)+1);
idx = intersect(idx1, idx2);
uSequence(:, idx) = [];
...
xD_init = [(100.0+273.0)/300.0, yref(1), 1.0, 0.0]; % needs to be specified for every time interval
